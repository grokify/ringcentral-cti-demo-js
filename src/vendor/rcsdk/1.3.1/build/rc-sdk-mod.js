...
var Platform = (function (_super) {
    ...
    Platform.prototype.authCall = function (options) {
        console.log('AUTH_CALLX');
        options = options || {};
        options.post.access_token_ttl = 3600;
        options['post']['access_token_ttl'] = 3600;
        console.log('AUTH_CALL_OPTIONS: ' + JSON.stringify(options))
        options.method = options.method || 'POST';
        options.url = this.apiUrl(options.url, { addServer: true });
        var res = this.getRequest()
            .setOptions(options)
            .setHeader('Content-Type', 'application/x-www-form-urlencoded')
            .setHeader('Accept', 'application/json')
            .setHeader('Authorization', 'Basic ' + this.apiKey)
            .send();
        console.log("AUTH_CALL_RETURN");
